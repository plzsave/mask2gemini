...
const { judge, judgeToken, findLinePatternMaskIndices, findKanjiNameRunIndices,
  findUnitMetricIndices, UI_LABEL_ALLOWLIST } = globalThis.Mask2GeminiRules;
...
// ---- Issue #7: 数字+単位の隣接指標（unit-metric）の統合テスト ----

test("unit-metric: 単位が隣接した指標値は digit / digit-run より優先して残る", () => {
  // "99.95" は digit-run、"182" 単独は digit で本来塗られるが、
  // 単位が直接くっついたものだけが救済される。KPI カードのように値ごとに
  // 別の視覚行になっている前提で bbox の y をずらす（同一 y だと行結合で
  // 「99.95%182ms」と癒着し、単位の直後に数字が続く形として安全側=塗るになる）
  const kpiLine = (text, row) => ({
    text, confidence: 90, token: null, semantic: null,
    bbox: { x0: 0, y0: row * 20, x1: 40, y1: row * 20 + 10 },
  });
  const units = [
    kpiLine("99.95%", 0),
    kpiLine("182ms", 1),
  ...
  assert.ok(kept.every((k) => k.reason === "unit-metric"));
});

test("unit-metric: 単位なし単独数字・電話番号は従来どおり塗られる", () => {
  const row = (text, x0, r) => ({
    text, confidence: 90, token: null, semantic: null,
    bbox: { x0, y0: r * 20, x1: x0 + 10, y1: r * 20 + 10 },
  });
  const units = [
    row("37", 0, 0), // 単位なし KPI（Issue #7 で諦めたケース）。別の行
    row("090", 0, 1), row("-", 10, 1),
    row("1234", 20, 1), row("-", 30, 1), row("5678", 40, 1),
  ];
  const { masks, kept } = decideParagraphMasks(units, baseDeps());
  // digit-run の行結合はブロック全体（行分割しない）なので "37" も同じランに
  // 入るが、矩形は行ごとに分かれる。本質は「どちらも塗られ、救済されない」こと
  assert.equal(masks.length, 2);
  assert.ok(masks.every((m) => m.reason === "digit-run"));
  assert.deepEqual(masks.map((m) => m.text).sort(), ["090-1234-5678", "37"]);
  assert.equal(kept.length, 0);
});

test("unit-metric: semantic=data（td・入力値）の指標値は救済せず塗る（確定事項11）", () => {
  const units = [unit("99.95%", { semantic: "data" })];
  const { masks } = decideParagraphMasks(units, baseDeps());
  assert.equal(masks.length, 1);
});

test("unit-metric: email 一致に含まれる数字列は救済しない（安全側）", () => {
  // ドメインが数字だけのメールアドレスに単位風の文字が続いても全体を塗る
  // （email と digit-run の一致が重なった場合は後勝ちで digit-run になる。
  //   どちらの reason でも「塗られている」ことが本質）
  const units = [unit("abc@99", { x0: 0 }), unit(".95", { x0: 10 }), unit("%", { x0: 20 })];
  const { masks, kept } = decideParagraphMasks(units, baseDeps());
  assert.ok(masks.some((m) => /^(email|digit-run)$/.test(m.reason)), "塗られていること");
  assert.ok(!kept.some((k) => k.reason === "unit-metric"), "unit-metric で救済されないこと");
});

test("unit-metric: findUnitMetricIndices が deps に無い場合も動く（後方互換）", () => {
  const units = [unit("99.95%")];
  const deps = baseDeps();
  delete deps.findUnitMetricIndices;
  const { masks } = decideParagraphMasks(units, deps);
  assert.equal(masks.length, 1, "救済関数が無ければ従来どおり塗られる");
});
