...
// ---- Issue #6: email-frag（折り返しで分断されたメール断片） ----

test("email-frag: @ で始まるドメイン断片は単独の行でも塗られる", () => {
  // 折り返し2行目が別 block になったケース（local-part が無いので email には一致しない）
  const units = [{ text: "@subdomain" }, { text: "." }, { text: "example" }, { text: "." }, { text: "org" }];
  const hits = findLinePatternMaskIndices(units);
  assert.equal(hits.size, units.length, "ドット等の区切りも含め全 unit が塗られること");
  for (const [, reason] of hits) assert.equal(reason, "email-frag");
});

test("email-frag: ローカルパート断片（3セグメント以上の連結）は塗られる", () => {
  const cases = [
    ["kenichi", ".", "tanaka", ".", "longname"],
    ["sayuri_kobayashi", "+", "work"], // _ と + の混在
    ["sayuri_kobayashi", "+", "work", "@"], // 行末の @ もランに含まれる（隙間対策）
    ["watanabe", ".", "contact", ".", "address"],
  ];
...
test("email-frag: 2セグメントの UI 語（error_code 等）は対象外", () => {
  // data-like 扱い（ラベル保護バイパス）が一般的な複合語に及ばないよう 3 セグメント以上に限定
  const cases = [
    [{ text: "error" }, { text: "_" }, { text: "code" }],
    [{ text: "sync" }, { text: "-" }, { text: "status" }],
    [{ text: "co" }, { text: "." }, { text: "jp" }],
  ];
  for (const units of cases) {
    const hits = findLinePatternMaskIndices(units);
    assert.equal(hits.size, 0, `${units.map((u) => u.text).join("")} はヒットしないこと`);
  }
});

test("email-frag: 完全なメールアドレスは従来どおり email として塗られる（断片に格下げされない）", () => {
  const units = [{ text: "kenichi.tanaka.longname@example-corp.co.jp" }];
  const hits = findLinePatternMaskIndices(units);
  assert.equal(hits.get(0), "email");
});
...
// ---- Issue #26: ひらがなの名（kana-name） ----
// 実測した kuromoji の品詞（tok26 検証: さくら=名詞/一般、姓=固有名詞/人名/姓、
// 敬称=名詞/接尾/人名、東京=固有名詞/地域）を模した疑似トークンで検証する
const jinmeiUnit = (surface) => ({
  text: surface,
  token: { surface_form: surface, pos: "名詞", pos_detail_1: "固有名詞",
    pos_detail_2: "人名", pos_detail_3: "姓", word_type: "KNOWN" },
});
const withDetail2 = (surface, pos, d1, d2) => ({
  text: surface,
  token: { surface_form: surface, pos, pos_detail_1: d1, pos_detail_2: d2, word_type: "KNOWN" },
});

test("kana-name: 岡本さくら（姓+ひらがな一般名詞）を検出する", () => {
  const units = [jinmeiUnit("岡本"), runUnit("さくら", "名詞", "一般")];
  const hits = findKanjiNameRunIndices(units);
  assert.equal(hits.size, 2, "姓と名の両方が塗られること");
  assert.equal(hits.get(1), "kana-name");
});

test("kana-name: 空白を挟んだ「中村 ゆき」も空白ごとランになる", () => {
  const units = [jinmeiUnit("中村"), spaceUnit(), runUnit("ゆき", "名詞", "固有名詞")];
  const hits = findKanjiNameRunIndices(units);
  assert.equal(hits.size, 3);
});

test("kana-name: 敬称（さん・様・ちゃん・くん = 名詞/接尾）は巻き込まない", () => {
  for (const honorific of ["さん", "様", "ちゃん", "くん"]) {
    const units = [jinmeiUnit("田中"), withDetail2(honorific, "名詞", "接尾", "人名")];
    assert.equal(findKanjiNameRunIndices(units).size, 0,
      `田中${honorific} はランにならないこと（田中単体は proper-noun が塗る）`);
  }
});

test("kana-name: アンカーは人名限定（東京みやげ = 地域+一般かな語は対象外）", () => {
  const units = [
    withDetail2("東京", "名詞", "固有名詞", "地域"),
    runUnit("みやげ", "名詞", "一般"),
  ];
  assert.equal(findKanjiNameRunIndices(units).size, 0);
});

test("kana-name: ひらがな語が先頭のとき（さくら保育園）はランにならない", () => {
  const units = [runUnit("さくら", "名詞", "一般"), runUnit("保育園", "名詞", "一般")];
  assert.equal(findKanjiNameRunIndices(units).size, 0);
});

test("kana-name: かな名の後に漢字が続く複合（岡本さくら保育園）は名までで切る", () => {
  const units = [
    jinmeiUnit("岡本"), runUnit("さくら", "名詞", "一般"), runUnit("保育園", "名詞", "一般"),
  ];
  const hits = findKanjiNameRunIndices(units);
  assert.equal(hits.size, 2, "保育園は巻き込まないこと");
  assert.ok(hits.has(0) && hits.has(1) && !hits.has(2));
});

test("kana-name: 5文字以上のひらがな語（さくらんぼ等）は名とみなさない", () => {
  const units = [jinmeiUnit("岡本"), runUnit("さくらんぼ", "名詞", "一般")];
  assert.equal(findKanjiNameRunIndices(units).size, 0);
});
