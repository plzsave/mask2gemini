...
16. （ワイヤーフレーム出力）既定では確認画面に④ボタンが出ないこと。設定でオンに
    すると DOM 経路で④が有効になり、保存した `.excalidraw` を excalidraw.com
    （または VS Code 拡張）で開いて編集できること。ファイル内にマスクした文字列が
    含まれないこと（E2E で機械検査）。OCR 経路では④が無効表示になること
...
14. ワイヤーフレーム出力（Issue #20・確定事項12）: dom-extractor の装飾抽出（decor）、
    wireframe-exporter.js（純関数＋単体テスト）、設定トグルと確認画面④ボタン、
