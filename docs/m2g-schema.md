...
| `screen` | **撮影された画面の範囲そのもの**（DOM の要素ではない合成矩形） | 画面の寸法・基調色の基準。UI 部品ではないのでダミー化・複製の対象にしない |

`decor` の色は元ページの実測値を sRGB の `#rrggbb[aa]` に正規化したもので、
`backgroundColor`（塗り）と `strokeColor`（枠線）を**別々に持つ**。

`screen` はファイル中に**必ず 1 枚だけ**、要素配列の**先頭（最背面）**に置かれる。
`width` / `height` が撮影時の画面サイズ（CSS px）、`backgroundColor` がページ全体の
地色で、**画面の基調色はここを見る**。座標系の原点 `(0, 0)` はこの矩形の左上に一致する。
`appState.viewBackgroundColor`（キャンバス＝画面外）は地色から明暗をずらした色で、
「画面の内と外」を塗り分けるためだけのものなので、配色の根拠にはしない
（`screen` を出せない経路では従来どおり地色がそのまま入る）。

ブロックの区切り線（見出しの下線・リスト行の罫線・左アクセント等）は、
**その色で塗られた細長い `decor` 矩形**として出る（高さまたは幅が数 px）。
元ページの `border-bottom` 等 1 辺だけの枠線がこの形になるため、
「細長い decor ＝ 区切り線」と読んでよい。元ページ側が画面全体を枠線で囲んで
いる場合、その枠は `backgroundColor: "transparent"` ＋ `strokeColor` 付きの
大きな `decor` 矩形として出る（画面の範囲を示す `screen` とは別物で、
こちらは実在の DOM 要素）。
