...
この規約のバージョン。現在は `1`。語彙（role / reason / kind の値）の**追加**では
上がらないので、**一覧にない値に出会っても無視せず「そういう種別がある」として
扱ってよい**（前方互換）。既存フィールドの意味が変わる非互換変更のときだけ上がる。
...
同じ列のヘッダ（`kind: "th"` の text）を見れば何の列か分かる。
