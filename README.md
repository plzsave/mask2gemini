...
- 抽出由来の全要素には `customData.m2g` に意味のメタデータが入る
  （スキーマバージョン `v`、`role: "masked"` + 判定種別、`"text"` / `"decor"` /
  `"icon"` 等）。DOM の要素種別が分かるテキスト・マスクには `kind`
  （`th` / `td` / `label` / `nav` / `h1` / `button` / `input:text` 等）、
  テーブル内の要素には `tableId` / `col`（セルのマスクと列ヘッダを結ぶ序数）も入る。
  **全語彙と読み分けの規約は [docs/m2g-schema.md](./docs/m2g-schema.md)**
  （配布 zip に同梱）にあり、LLM/AI Agent にファイルを読ませるときは
  この文書ごとコンテキストに貼り付けて使う。要点は「マスク矩形の `reason`・`kind` が
  何のダミーを入れる枠かを示す」「**`customData` の無い要素 = 人間が後から追加した
  提案部分**」の 2 つ。`v` は語彙追加では上がらず、非互換変更のときだけ上がる。
