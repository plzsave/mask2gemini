...
判定は形態素トークン単位（kuromoji.js・ローカル実行）で行い、**固有名詞（人名・地名・組織名）と未知語は辞書登録なしで塗られる**。加えて `extension/rules.js` の定数（アローリスト辞書・文字数閾値・正規表現）で調整できる。判定は recall 優先（迷ったら塗る）に倒してある。OCR は精度向上のため画像を2倍に拡大してから実行する。
